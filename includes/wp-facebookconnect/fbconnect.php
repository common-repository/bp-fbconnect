...
function fbc_init_auth() {
  ...
  if ($assoc_fbuid) {
    if ($fbuid === $assoc_fbuid) {
      ...
      wp_logout();
      header('Location: ' . js_escape( $_SERVER['REQUEST_URI'] ) );
      exit();
    }
  } else {
     if ($user->ID) {
...
function this_plugin_path() {
  ...
  return apply_filters( 'fbconnect_plugin_path', get_option('siteurl').'/'. PLUGINDIR .'/' . array_pop($path) );
}
...
function fbc_get_avatar($avatar, $id_or_email, $dims, $size, $default = false ) {
